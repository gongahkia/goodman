...
> Historical planning artifact only. This document does not define the supported surface of the current Chrome-first branch.
...
> This PRD captured an earlier multi-browser product direction, including Firefox packaging and polyfill-based portability. Those items are deferred on the current branch and remain here only as historical planning context.
...
**DONE WHEN**
- [x] `npm run build` produces a `dist/` folder with valid `manifest.json`, bundled JS for background, content, popup, and options.
- [x] Extension loads in `chrome://extensions` with developer mode without errors.
- [x] `chrome.storage.local` round-trip test: calling `setConfig({captureMode: 'region'})` then `getConfig()` returns `captureMode: 'region'` with all other fields populated from defaults.
...
**DONE WHEN**
- [x] Popup sends `GET_STATUS` to background and receives `STATUS` response (verify via console logs in background service worker and popup DevTools).
- [x] Content script receives messages sent via `sendToTab` from background (verify via console log on a quiz page).
...
**DONE WHEN**
- [x] Full-page screenshots are captured as valid base64 PNGs.
- [x] Region selection UI allows click-and-drag with resize handles.
- [x] Cropped region screenshots match the selected bounding rect.
- [x] Last-used region persists per domain and restores on revisit.
...
**DONE WHEN**
- [x] Interface is defined in `llm/provider.ts`.
- [x] Both `OllamaProvider` and `OpenAICompatProvider` implement it.
- [x] `QuizAnswer` type is exported and used by the orchestrator and overlay.
...
**DONE WHEN**
- [x] With Ollama running locally, `isAvailable()` returns `true`.
- [x] Ollama with llava/qwen2.5-vl can analyze a screenshot and return a structured `QuizAnswer`.
- [x] With Ollama stopped, `isAvailable()` returns `false` (does not throw).
- [x] Model selection lists only vision-capable models from `/api/tags`.
...
**DONE WHEN**
- [x] Can analyze screenshots via any OpenAI-compatible local server.
- [x] Presets auto-fill the correct base URL for each backend.
- [x] Works without an API key when the server doesn't require one.
- [x] Custom base URL works for non-preset servers.
...
**DONE WHEN**
- [x] `createProvider` returns correct provider type for each `llmProvider` value.
- [x] `autoDetect()` finds a running Ollama instance and returns a configured provider.
- [x] `autoDetect()` falls back to LM Studio, then other presets, if Ollama is not running.
- [x] Calling `createProvider` with the same config returns a cached instance.
...
**DONE WHEN**
- [x] All listed platforms are correctly detected from their URLs.
- [x] Platform-specific prompt hints are returned for each known platform.
- [x] Unknown URLs return `'generic'` with a generic prompt hint.
- [x] Platform detection is case-insensitive.
...
**DONE WHEN**
- [x] Manual shortcut (Alt+Q) captures and analyzes the current page.
- [x] Known quiz platforms are detected from URL and badge is shown.
- [x] Auto-capture toggle works per-platform when enabled.
- [x] Badge clears when navigating away from a known platform.
...
**DONE WHEN**
- [x] End-to-end flow works: shortcut triggers screenshot, LLM analyzes, answer is displayed in overlay.
- [x] Platform-specific prompt hints are appended when on a known platform.
- [x] Session history is updated after each analysis.
- [x] Errors in any stage are caught and displayed to the user.
...
**DONE WHEN**
- [x] Receiving `REQUEST_ANSWER` with a valid screenshot triggers LLM call and returns `ANSWER_READY` with populated `QuizAnswer`.
- [x] LLM errors are caught and returned as error `QuizAnswer` (no unhandled promise rejections in background).
- [x] Session log is appended on each question/answer cycle.
- [x] `GET_STATUS` returns the most recent answer and provider status.
...
**DONE WHEN**
- [x] Answer displays in a clean floating overlay on the page.
- [x] Confidence bar is color-coded correctly (green/yellow/red).
- [x] Panel is draggable to any position and dismissable via X or Escape.
- [x] Shadow DOM isolates styles from the host page.
- [x] Fade in/out animations are smooth and non-janky.
...
**DONE WHEN**
- [x] Popup opens showing current provider status (green/red indicator).
- [x] Last answer is displayed with question type, answer text, confidence badge, and collapsible reasoning.
- [x] "Capture Full Page" and "Capture Region" buttons trigger the corresponding capture mode.
- [x] "Settings" button opens the options page.
...
**DONE WHEN**
- [x] Selecting each provider shows the correct input fields.
- [x] Ollama model dropdown lists only vision-capable models.
- [x] OpenAI-compatible presets auto-fill the correct base URL.
- [x] "Test Connection" shows "Connected" or a descriptive error.
- [x] All settings persist to `chrome.storage.local` and send `CONFIG_UPDATED`.
- [x] Model recommendations are displayed with VRAM requirements.
...
**DONE WHEN**
- [x] After 3 question/answer cycles, log viewer shows 3 entries in reverse chronological order.
- [x] "Clear Log" empties the log and re-renders an empty state.
- [x] "Export Log" downloads a `.json` file containing the `LogEntry[]` array.
- [x] Toggling between answer panel and log panel works without losing state.
...
**DONE WHEN**
- [x] Recommendations display with VRAM requirements for each tier.
- [x] Already-downloaded models are visually highlighted.
- [x] "Pull Model" button triggers Ollama model download and shows progress.
- [x] Non-Ollama providers show recommendations without the pull button.
...
**DONE WHEN**
- [x] LLM timeout after 30s triggers `LLM_TIMEOUT` error shown in popup.
- [x] Capture failure shows `CAPTURE_FAILED` in popup.
- [x] After an error, the trigger listener continues to work (no crash).
- [x] Errors include both developer-facing (code + stack) and user-facing (message) info.
...
**DONE WHEN**
- [x] A transient network error on first attempt succeeds on retry (verify with mock/spy).
- [x] 4xx errors are NOT retried — fail immediately.
- [x] After `maxRetries` exhausted, the original error propagates.
- [x] Backoff delays are correct: 1000ms first retry, 2000ms second retry.
...
**DONE WHEN**
- [x] After answering 3 questions, `getSessionLog()` returns 3 `LogEntry` items.
- [x] On browser restart, session log is empty.
- [x] `exportSessionLog()` returns valid JSON string.
- [x] 501st entry causes the oldest entry to be removed (array stays at 500).
...
**DONE WHEN**
- [x] No LLM API fetch calls exist in content script code.
- [x] `getMaskedConfig()` returns a safe display version of the config.
- [x] CSP header is set in manifest.
...
**DONE WHEN**
- [x] `npm run dev` starts a watch process; editing a `.ts` file triggers rebuild within 2s.
  <!-- WHY 2s rebuild: acceptable developer feedback loop; most esbuild/vite rebuilds complete in <500ms -->
- [x] Extension auto-reloads in Chrome after rebuild (verify via console log timestamp).
- [x] `npm run build:prod` outputs minified bundle without dev-reload code.
...
**DONE WHEN**
- [x] `npm run lint` passes with no errors on the codebase.
- [x] `npm run typecheck` passes with no TypeScript errors.
- [x] Introducing `let x: any` triggers a lint warning.
...
**DONE WHEN**
- [x] `npm test` runs all test files and passes.
- [x] Response parsing tests cover all parsing strategies.
- [x] Platform detection tests verify all supported platforms.
...
**DONE WHEN**
- [x] Content script loads without console errors on any page.
- [x] Keyboard shortcut triggers capture → background → overlay display pipeline.
- [x] `CONFIG_UPDATED` messages update content script behavior without requiring page reload.
- [x] `ANSWER_READY` messages from background are rendered in the overlay.
...
- [x] All message types are handled (`REQUEST_ANSWER`, `GET_STATUS`, `CONFIG_UPDATED`, `CAPTURE_SCREENSHOT`).
- [x] LLM calls are wrapped in retry logic.
- [x] Session log is cleared on extension install/activate.
...
**DONE WHEN**
- [x] Popup opens and displays current status from background.
- [x] Real-time updates: answering a question while popup is open updates the display.
- [x] Log viewer toggle works and shows session history.
- [x] On any quiz platform with a local LLM running: shortcut pressed → screenshot captured → answer appears in overlay within 10s.
- [x] Switching providers in options takes effect on next capture without page reload.
- [x] Session log contains the Q&A entry after the cycle.
- [x] Errors (e.g., Ollama down) display in popup and overlay with actionable message.
...
**DONE WHEN**
- [x] Pressing `Alt+Q` on any page triggers the capture and analysis pipeline.
- [x] Works on any website, not just known quiz platforms.
- [x] Shortcut is visible in `chrome://extensions/shortcuts`.
...
**DONE WHEN**
- [x] After answering a question with 90% confidence, badge shows "90%" in green.
- [x] After an error, badge shows "!" in red.
- [x] Navigating away from a quiz platform clears the badge.
...
**DONE WHEN**
- [x] `npm run build:firefox` produces a `dist/` loadable in Firefox as temporary add-on.
- [x] Core flow (capture → LLM → overlay display) works in Firefox.
- [x] No direct `chrome.*` API calls remain in source (all go through `browser-api.ts`).
...
**DONE WHEN**
- [x] CI passes on push and PR.
- [x] Failing lint/typecheck/test blocks merge.
...
**DONE WHEN**
- [x] `npm run package` produces a valid .zip loadable in Chrome.
- [x] `npm run package:firefox` produces a valid .xpi loadable in Firefox.
...
- [x] Installation instructions work on a clean Chrome + Ollama setup.
...
**DONE WHEN**
- [x] Integration test passes with mocked Chrome APIs and LLM.
- [x] Error path test verifies error handling works.
